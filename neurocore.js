...
(function (global) {
  ...
  /* ════════════════════════════════════════════════════════════════
     ⑩ MESSAGE STORE — Sorted per-chat in-memory store (Layer 1)

     Holds ALL loaded messages for every chat visited this session.
     Never pruned from memory — only the DOM window is pruned.
     Memory is cheap; DOM is the expensive part.

     API:
       upsert(chatId, msg)           binary-insert or merge-update
       upsertMany(chatId, msgs)      bulk insert (idempotent)
       remove(chatId, id)            delete by id
       getAll(chatId)                sorted array ref (do not mutate)
       has(chatId, id)               O(1) membership check
       indexOf(chatId, id)           index in sorted array
       count(chatId)                 message count for chat
       clearChat(chatId)             evict a single chat
       clearAll()                    full teardown
  ════════════════════════════════════════════════════════════════ */
  const MessageStore = (function () {
    // chatId → { msgs: Array<msg> sorted ascending by .time, ids: Set<string> }
    const _C = new Map();

    function _c(id) {
      if (!_C.has(id)) _C.set(id, { msgs: [], ids: new Set() });
      return _C.get(id);
    }
    ...
    function upsert(chatId, msg) {
      const c = _c(chatId);
      if (c.ids.has(msg.id)) {
        // Merge-update: keep existing fields, overwrite with incoming
        const i = c.msgs.findIndex(m => m.id === msg.id);
        if (i >= 0) c.msgs[i] = Object.assign({}, c.msgs[i], msg);
        return;
      }
      // Binary-insert by .time (ascending)
      const t = msg.time || 0;
      let lo = 0, hi = c.msgs.length;
      while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        c.msgs[mid].time <= t ? (lo = mid + 1) : (hi = mid);
      }
      c.msgs.splice(lo, 0, msg);
      c.ids.add(msg.id);
    }

    function upsertMany(chatId, msgs) {
      if (!msgs || !msgs.length) return;
      msgs.forEach(m => upsert(chatId, m));
    }

    function remove(chatId, id) {
      const c = _C.get(chatId);
      if (!c || !c.ids.has(id)) return false;
      const i = c.msgs.findIndex(m => m.id === id);
      if (i >= 0) c.msgs.splice(i, 1);
      c.ids.delete(id);
      return true;
    }

    function getAll(chatId)      { return _C.has(chatId) ? _C.get(chatId).msgs : []; }
    function has(chatId, id)     { const c = _C.get(chatId); return c ? c.ids.has(id) : false; }
    function count(chatId)       { return _C.has(chatId) ? _C.get(chatId).msgs.length : 0; }
    function indexOf(chatId, id) { const c = _C.get(chatId); return c ? c.msgs.findIndex(m => m.id === id) : -1; }
    function clearChat(chatId)   { _C.delete(chatId); }
    function clearAll()          { _C.clear(); }

    return { upsert, upsertMany, remove, getAll, has, count, indexOf, clearChat, clearAll };
  }());

  /* ════════════════════════════════════════════════════════════════
     ⑪ VIRTUALIZATION ENGINE — Anchored Bidirectional DOM Window

     Correct architecture (Layer 2):

       MessageStore (ALL messages in JS memory — never trimmed)
         ↓
       VirtualizationEngine (DOM window: indices wStart..wEnd)
         ↓
       DOM:  [chatIntro?] [_vt spacer] [~80 rendered .mw nodes] [_vb spacer]

     Key invariants:
     • MessageStore is the source of truth. Memory only.
     • DOM holds at most WINDOW_SIZE = 80 messages at once.
     • Spacers simulate scroll height of off-screen messages so
       scroll position is smooth and physics stays intact.
     • ResizeObserver builds a per-message height cache for accurate
       spacer calculations (essential for KaTeX / dynamic heights).
     • Top-trimming: messages that scroll 400px above the viewport
       are removed from DOM; top spacer grows to compensate.
       scrollTop is restored synchronously — no visible jump.
     • Anchor preservation on prepend: scroll position is corrected
       by measuring the first visible .mw before and after insertion.
     • Deletion: direct node.remove() + store removal. No rerender.

     Public API:
       init(chatId)                  create spacers + ResizeObserver
       activate(chatId, box, cb)     inject spacers, set window, attach scroll
       observe(chatId, el)           RO-observe a rendered node
       onPrepend(chatId, box, count) expand window upward + re-inject spacers
       onAppend(chatId, box)         update wEnd, trim top if needed
       del(chatId, msgId, box)       remove from store + DOM
       storedAboveWindow(chatId)     messages in store above DOM window
       getWindow(chatId)             { start, end }
       detachScroll(chatId)          remove scroll listener (keep state)
       reset(chatId)                 full teardown for chat switch
  ════════════════════════════════════════════════════════════════ */
  const VirtualizationEngine = (function () {
    const WINDOW_SIZE = 80;   // max .mw nodes in DOM simultaneously
    const GAP_PX      = 3;    // matches .msgs { gap: 3px } in CSS
    const AVG_H       = 72;   // fallback height per message when not yet measured
    const TRIM_BUF    = 400;  // px above viewport before a node is eligible for trim

    // chatId → { wStart, wEnd, hCache: Map<id,px>, ro: ResizeObserver, scrollOff: fn|null }
    const _S  = new Map();
    // chatId → { top: HTMLElement, bot: HTMLElement }
    const _SP = new Map();

    function _s(id) {
      if (!_S.has(id)) _S.set(id, { wStart: 0, wEnd: 0, hCache: new Map(), ro: null, scrollOff: null });
      return _S.get(id);
    }

    /* ── Recalculate both spacer heights from height cache ── */
    function _recalc(chatId) {
      const s  = _S.get(chatId);
      const sp = _SP.get(chatId);
      if (!s || !sp) return;
      const msgs = MessageStore.getAll(chatId);
      let tH = 0;
      for (let i = 0; i < s.wStart; i++) tH += (s.hCache.get(msgs[i]?.id) || AVG_H);
      let bH = 0;
      for (let i = s.wEnd; i < msgs.length; i++) bH += (s.hCache.get(msgs[i]?.id) || AVG_H);
      sp.top.style.height = tH + 'px';
      sp.bot.style.height = bH + 'px';
    }

    /* ── Inject/reposition spacers in correct DOM positions ──
       _vt goes immediately before the first .date-div or .mw[data-mid].
       _vb goes at the very end of box.
       Safe to call multiple times (idempotent repositioning). ── */
    function _inject(chatId, box) {
      const sp = _SP.get(chatId);
      if (!sp) return;
      const firstContent = box.querySelector('.date-div, .mw[data-mid]');
      if (firstContent && firstContent !== sp.top) {
        box.insertBefore(sp.top, firstContent);
      } else if (!box.contains(sp.top)) {
        box.append(sp.top);
      }
      if (box.lastChild !== sp.bot) box.append(sp.bot);
    }

    /* ── Remove DOM nodes that have scrolled far above the viewport ──
       After removal, scrollTop is restored so the user sees no jump.
       The top spacer grows to compensate the removed pixel height. ── */
    function _trimTop(chatId, box) {
      const s = _S.get(chatId);
      if (!s || s.wStart >= s.wEnd) return;

      const mwEls  = box.querySelectorAll('.mw[data-mid]');
      if (!mwEls.length) return;
      const boxTop = box.getBoundingClientRect().top;

      let trimCount = 0;
      for (const el of mwEls) {
        if (el.getBoundingClientRect().bottom < boxTop - TRIM_BUF) trimCount++;
        else break; // DOM is in time-ascending order; stop at first visible
      }
      if (trimCount === 0) return;

      const prevST = box.scrollTop;
      for (let i = 0; i < trimCount; i++) mwEls[i]?.remove();
      s.wStart = Math.min(s.wStart + trimCount, s.wEnd);
      _recalc(chatId);
      // Spacer grew by same amount removed → scrollHeight unchanged → no jump
      box.scrollTop = prevST;
    }

    /* ════ Public API ════ */

    /**
     * init — create spacer elements + ResizeObserver for this chat.
     * Must be called before first render (before appendMsg loop).
     * Does NOT inject spacers into the DOM yet — call activate() for that.
     */
    function init(chatId) {
      const s = _s(chatId);

      // Create or reset spacer elements
      if (!_SP.has(chatId)) {
        const top = document.createElement('div');
        top.className = '_vt';
        top.setAttribute('aria-hidden', 'true');
        top.style.cssText = 'height:0px;width:100%;flex-shrink:0;contain:strict;';
        const bot = document.createElement('div');
        bot.className = '_vb';
        bot.setAttribute('aria-hidden', 'true');
        bot.style.cssText = 'height:0px;width:100%;flex-shrink:0;contain:strict;';
        _SP.set(chatId, { top, bot });
      } else {
        // Reset heights on re-init
        const sp = _SP.get(chatId);
        sp.top.style.height = '0px';
        sp.bot.style.height = '0px';
      }

      // Wire ResizeObserver: update height cache when messages resize
      // (KaTeX re-renders, image loads, collapsible expansions, etc.)
      ...
      s.ro = new ResizeObserver(entries => {
        ...
        for (const e of entries) {
          ...
          if (!mid) continue;
          const h = e.contentRect.height + GAP_PX;
          if (s.hCache.get(mid) !== h) { s.hCache.set(mid, h); changed = true; }
        }
        if (changed) _recalc(chatId);
      });
      // Reset window indices and height cache for fresh start
      s.wStart = 0;
      s.wEnd   = 0;
      s.hCache.clear();
    }
    ...
    /**
     * activate — call AFTER the first render loop.
     * Injects spacers, sets window to show the last WINDOW_SIZE messages,
     * removes excess top nodes, recalcs spacers, and attaches the scroll handler.
     *
     * @param {string}   chatId
     * @param {HTMLElement} box  — the #msgs scroll container
     * @param {Function} onNearTop — called when user scrolls near top of content
     */
    function activate(chatId, box, onNearTop) {
      _inject(chatId, box);
      ...
      // Trim excess top nodes if more were rendered than WINDOW_SIZE
      const mwEls   = box.querySelectorAll('.mw[data-mid]');
      const excess  = Math.max(0, mwEls.length - WINDOW_SIZE);
      for (let i = 0; i < excess; i++) mwEls[i]?.remove();

      s.wStart = Math.max(0, total - WINDOW_SIZE);
      s.wEnd   = total;
      _recalc(chatId);

      // Attach scroll handler (throttled via rAF)
      if (s.scrollOff) s.scrollOff();
      let _busy = false;
      const fn = () => {
        if (_busy) return;
        _busy = true;
        requestAnimationFrame(() => {
          _busy = false;
          // Near-top detection: is the first rendered .mw within 400px of container top?
          const firstMw = box.querySelector('.mw[data-mid]');
          if (firstMw) {
            const bRect = box.getBoundingClientRect();
            const fRect = firstMw.getBoundingClientRect();
            if (fRect.top < bRect.top + 400) onNearTop && onNearTop();
          }
          _trimTop(chatId, box);
        });
      };
      box.addEventListener('scroll', fn, { passive: true });
      s.scrollOff = () => box.removeEventListener('scroll', fn);
    }

    /** Observe a rendered .mw node with the ResizeObserver. */
    function observe(chatId, el) {
      const s = _S.get(chatId);
      if (s?.ro && el) s.ro.observe(el);
    }

    /**
     * onPrepend — call after prepending older messages.
     * Expands the window upward by `count`, repositions _vt,
     * and recalculates spacer heights.
     */
    function onPrepend(chatId, box, count) {
      const s = _s(chatId);
      s.wStart = Math.max(0, s.wStart - count);
      _inject(chatId, box);  // reposition _vt before the new first message
      _recalc(chatId);
    }

    /**
     * onAppend — call after appending new messages at the bottom.
     * Updates wEnd, trims top if window is over capacity, recalcs spacers.
     */
    function onAppend(chatId, box) {
      const s = _s(chatId);
      s.wEnd = MessageStore.count(chatId);
      _trimTop(chatId, box);
      _recalc(chatId);
    }

    /**
     * del — remove a message from both MessageStore and DOM.
     * Direct node.remove() — no re-render of any other message.
     */
    function del(chatId, msgId, box) {
      MessageStore.remove(chatId, msgId);
      if (box) {
        const el = box.querySelector('[data-mid="' + msgId + '"]');
        if (el) el.remove();
      }
      const s = _s(chatId);
      const n = MessageStore.count(chatId);
      s.wEnd   = Math.min(s.wEnd,   n);
      s.wStart = Math.min(s.wStart, s.wEnd);
      _recalc(chatId);
    }

    /** Number of messages in store that sit above the current DOM window start. */
    function storedAboveWindow(chatId) { return _s(chatId).wStart; }

    /** Current DOM window range as MessageStore indices. */
    function getWindow(chatId) {
      const s = _s(chatId);
      return { start: s.wStart, end: s.wEnd };
    }

    /** Detach the scroll listener while preserving all other state. */
    function detachScroll(chatId) {
      const s = _S.get(chatId);
      if (s?.scrollOff) { s.scrollOff(); s.scrollOff = null; }
    }

    /**
     * reset — full teardown for a chat.
     * Disconnects ResizeObserver, detaches scroll listener,
     * removes state from maps. Does NOT touch MessageStore.
     */
    function reset(chatId) {
      const s = _S.get(chatId);
      if (s) {
    ...
    return {
      init, activate, observe,
      onPrepend, onAppend, del,
      storedAboveWindow, getWindow,
      detachScroll, reset,
    };
  }());
