...
async function _deleteMessage(msgId) {
  try {
    ...
    if (!deleteDoc || !doc || !window.db || !msgId) return;
    const col = window._chatCollection || 'messages';
    await deleteDoc(doc(window.db, col, msgId));
    _log(`AI: Deleted message ${msgId} from Firestore.`);
  } catch (e) {
