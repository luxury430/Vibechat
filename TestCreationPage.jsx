...
function TestCreationPage({ config, onClose }) {
  ...
  const [questions, setQuestions] = useState(() => {
    // Resume from draft if existingQuestions supplied
    if (config.existingQuestions && config.existingQuestions.length) {
      return config.existingQuestions;
    }
  ...
  const handlePublish = useCallback(async () => {
    ...
    // ── 2. Validate — every question must have text, correct answer, and ≥ 2 options ──
    ...
    if (incomplete.length > 0) {
      if (typeof window.toast === 'function') {
        window.toast(
          `⚠ ${incomplete.length} question${incomplete.length > 1 ? 's' : ''} still incomplete`
        );
      }
      return;
    }
