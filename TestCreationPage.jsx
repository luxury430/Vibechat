...
(() => {
  const initTestCreation = () => {
    // Wait for the ES module to attach React to the window
    if (!window.React || !window.ReactCreateRoot) {
      setTimeout(initTestCreation, 50);
      return;
    }
...
/* ═══════════════════════════════════════════════════════════════════════
   OPTION ROW
════════════════════════════════════════════════════════════════════════ */
function OptionRow({ label, value, onChange, onFocus, isCorrect, onToggleCorrect, inputRef, preview }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
      ...
      {/* Preview: KaTeX render — Edit: text input */}
      {preview ? (
        <div
          style={{
            flex: 1,
            minHeight: 34,
            padding: '6px 11px',
            background: isCorrect ? 'rgba(99,102,241,.07)' : 'var(--bg-input)',
            border: `1px solid ${isCorrect ? 'rgba(99,102,241,.35)' : 'var(--border2)'}`,
            borderRadius: 8,
            color: value ? 'var(--tp)' : 'var(--tm)',
            fontSize: '.83rem',
            lineHeight: 1.6,
            overflowX: 'auto',
          }}
          dangerouslySetInnerHTML={value ? parseMath(value) : { __html: `<span style="opacity:.4">Option ${label}</span>` }}
        />
      ) : (
        <input
          ref={inputRef}
...
/* ═══════════════════════════════════════════════════════════════════════
   QUESTION CARD
════════════════════════════════════════════════════════════════════════ */
function QuestionCard({ index, total, chapter, data, onChange, entryDelay, onFocusField, onOpenNotation }) {
  ...
  // FIX: Safely trim values so whitespace doesn't falsely pass validation
  const hasQuestion  = (data.question || '').trim().length > 0;
  const filledOpts   = Object.values(data.options || {}).filter(opt => (opt || '').trim().length > 0).length;
  const isComplete   = hasQuestion && data.correct && filledOpts >= 2;

  /* Only silently tracks which field/ref is active — does NOT open the panel */
  const handleFocus = useCallback((field) => {
    onFocusField(index, field, refMap[field]);
  }, [index, onFocusField]);
...
function TestCreationPage({ config, onClose }) {
  ...
  /* questions state — FIX: always strictly matches numQ length */
  const [questions, setQuestions] = useState(() => {
    let initial = [];
    if (config.existingQuestions && config.existingQuestions.length) {
      initial = [...config.existingQuestions];
    }
    // Pad up to numQ if short
    if (initial.length < numQ) {
      const pad = Array.from({ length: numQ - initial.length }, (_, i) => ({
        id: initial.length + i,
        question: '',
        options: { A: '', B: '', C: '', D: '' },
        correct: null,
      }));
      initial = initial.concat(pad);
    }
    // Clamp to numQ if somehow longer
    return initial.slice(0, numQ);
  });

  /* publish state: 'idle' | 'publishing' | 'done' | 'error' */
  const [publishState, setPublishState] = useState('idle');

  /* stable draft ID — shared across autosave cycles */
  const draftId = useRef(
    config.draftId || `draft_${Date.now()}_${Math.random().toString(36).slice(2)}`
  ).current;
  ...
  /* completion count — FIX: safe trim mirrors QuestionCard isComplete */
  const completedCount = useMemo(() =>
    questions.filter(q =>
      (q.question || '').trim() &&
      q.correct &&
      Object.values(q.options || {}).filter(opt => (opt || '').trim().length > 0).length >= 2
    ).length,
    [questions]
  );

  /* ── Draft autosave — fires 8 s after last change while idle ── */
  useEffect(() => {
    if (publishState !== 'idle') return; // Don't autosave during publish
    const timer = setTimeout(() => {
      ...
      if (typeof window.testFeedSaveDraft === 'function') {
        window.testFeedSaveDraft(draft);
      }
    }, 8000);
    return () => clearTimeout(timer);
  }, [questions, publishState]);
  ...
  const handlePublish = useCallback(async () => {
    // ── 1. Guard: don't double-publish ──
    if (publishState !== 'idle') return;

    // ── 2. Validate & pinpoint the exact failing question ──
    const firstIncompleteIdx = questions.findIndex(q =>
      !(q.question || '').trim() ||
      !q.correct ||
      Object.values(q.options || {}).filter(opt => (opt || '').trim().length > 0).length < 2
    );
    ...
    if (firstIncompleteIdx !== -1) {
      const q = questions[firstIncompleteIdx];
      let reason = 'is incomplete';
      if (!(q.question || '').trim()) {
        reason = 'is missing the question text';
      } else if (Object.values(q.options || {}).filter(opt => (opt || '').trim().length > 0).length < 2) {
        reason = 'needs at least 2 options filled in';
      } else if (!q.correct) {
        reason = 'needs a correct answer selected (tap the A/B/C/D circle)';
      }

      if (typeof window.toast === 'function') {
        window.toast(`⚠ Q${firstIncompleteIdx + 1} ${reason}.`);
      }

      // Scroll to the specific card and flash its border red
      const cardEls = document.querySelectorAll('.tcp-card');
      if (cardEls[firstIncompleteIdx]) {
        const el = cardEls[firstIncompleteIdx];
    ...
    try {
      ...
      if (window.db && window._firebaseModExports) {
        const { addDoc, collection } = window._firebaseModExports;
        const docRef = await addDoc(
          collection(window.db, 'community_tests'),
          { ...testData, publishedAt: new Date() }
        );
      ...
      // ── 5. Add to local My Tests cache + update explore feed ──
      if (typeof window.testFeedAddMyTest === 'function') {
        window.testFeedAddMyTest(fullTestData);
      }

      // ── 6. Remove draft (it's now published) ──
      if (typeof window.testFeedDeleteDraft === 'function') {
        window.testFeedDeleteDraft(draftId);
      }

      // ── 7. Success feedback ──
      setPublishState('done');
      if (typeof window.toast === 'function') {
        window.toast('🎉 Test published to community!');
      }

      // ── 8. Close after brief success state ──
      setTimeout(handleClose, 900);

    } catch (err) {
      console.error('[NeuroForge] Publish failed:', err);
      setPublishState('error');
      if (typeof window.toast === 'function') {
        window.toast('Publish failed — check connection. Draft saved locally.');
      }
      // Reset so user can retry
      setTimeout(() => setPublishState('idle'), 2500);
    }
  }, [publishState, questions, chapter, title, numQ, xpPerQ, createdBy, draftId, handleClose]);
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 11 }}>
          ...
          <button
            ...
            className={`tcp-pub${publishState === 'publishing' ? ' tcp-pub-publishing' : ''}${publishState === 'done' ? ' tcp-pub-done' : ''}${publishState === 'error' ? ' tcp-pub-error' : ''}`}
            ...
            style={{
              ...
              cursor: publishState === 'idle' || publishState === 'error' ? 'pointer' : 'default',
      ...
      >
        ...
        <div style={{
          ...
          <span style={{
            ...
            color: 'rgba(74,222,128,.55)', fontSize: '.6rem',
          }}>
            <span style={{
...
window.unmountTestCreation = function() {
  ...
  }; // end initTestCreation

  initTestCreation(); // kick off polling
})(); // end IIFE
