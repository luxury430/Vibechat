/* globals: React, math (mathjs), and Plotly are set by scripts in chat.html */
const { useState, useEffect, useRef, useMemo, useCallback } = React;

// ── Safely get libraries at runtime (not at parse time) so a slow CDN
//    script cannot crash the whole component tree before React mounts. ──
...
const safeEval = (expr) => {
  ...
  try {
    const _math = getMath();
    if (!_math) return parseFloat(String(expr)) || 0; // mathjs not loaded yet
    const r = _math.evaluate(String(expr).trim());
...
function Graph({ data, layout }) {
  ...
  useEffect(() => {
    ...
    const P = (() => { const _P = getPlotly(); return _P?.newPlot ? _P : _P?.default; })();
    if (!P) return;
    const ly = {
...
// ─── Main App ─────────────────────────────────────────────────
function App() {
  // ── Wait for mathjs to be ready before rendering so safeEval never
  ...
  useEffect(() => {
    ...
    const to = setTimeout(() => { clearInterval(iv); setLibsReady(true); }, 6000); // failsafe
  ...
  if (!libsReady) {
    return (
      <div style={{ display:'flex', justifyContent:'center', alignItems:'center',
                    ...
                    background:'#0a0a1a', color:'#FF6B6B', fontFamily:'JetBrains Mono,monospace' }}>
...
// Register for use by openMathFormula()
window._MathFormulaExplorerComponent = App;
