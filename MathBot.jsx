...
const STYLE = `
  /* ── DESIGN TOKENS ─────────────────────────────────────────────────────────── */
  /* Dark is the default — matches NeuroForge dark vars exactly */
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Root wrapper — must not clip scroll */
  [data-theme] {
    min-height: 100%;
    overflow-y: auto;
    overflow-x: hidden;
  ...
  /* ── THEME TOGGLE (Hidden — app handles this globally) ── */
  ...
  .mode-card-top { display: flex; align-items: center; gap: 14px; margin-bottom: 12px; }
  .mode-glyph { display: none; }
  .mode-label { font-family: var(--font-d); font-size: 1.05rem; font-weight: 700; letter-spacing: -0.2px; color: var(--tp) !important; }
  .mode-sub { font-size: 0.8rem; color: var(--ts); font-weight: 500; margin-top: 2px; font-family: var(--font-b); }
  .mode-icon { font-size: 16px; margin-left: auto; opacity: 0.5; color: var(--ts) !important; transition: transform var(--transition); }
  .mode-card:hover .mode-icon { opacity: 1; color: var(--accent) !important; transform: translateX(4px); }
  .mode-topics {
    font-size: 0.72rem; color: var(--tm) !important; letter-spacing: 0.2px;
    font-family: var(--font-b); line-height: 1.6;
  }
  .mode-divider { display: none; }

  /* ── STUDY SESSION ── */
  .game-wrap { width: 100%; max-width: 540px; padding-bottom: 40px; }
  ...
  .back-btn {
    ...
    background: var(--bg-input); border: 1px solid var(--border2);
    font-family: var(--font-b); font-weight: 600; padding: 7px 14px; border-radius: var(--r-f);
    transition: all var(--transition);
  }
  ...
  .stat-num { font-size: 1.3rem; font-weight: 700; font-family: var(--font-b); line-height: 1; color: var(--tp) !important; }
  ...
  /* ── Question Card ── */
  .q-card {
    border-radius: var(--radius-md); border: 1px solid var(--border2) !important;
    padding: 24px 26px; margin-bottom: 16px; background: var(--bg-card);
    transition: border-color 0.3s; position: relative;
  }
  .q-card.ev-correct { border-color: #10b981 !important; box-shadow: 0 0 0 1px rgba(16,185,129,0.2); }
  .q-card.ev-wrong   { border-color: #f87171 !important; box-shadow: 0 0 0 1px rgba(239,68,68,0.2); }
  ...
  .q-text {
    ...
    letter-spacing: 0; white-space: pre-wrap; color: var(--tp);
  ...
  .option-btn {
    ...
    background: var(--bg-input); border: 1px solid var(--border2);
    color: var(--tp); font-size: 0.9rem; font-family: var(--font-b);
    font-weight: 600; transition: all var(--transition);
  }
  .option-btn:not(:disabled):hover { border-color: var(--accent); background: var(--bg-hover); }
  ...
  .option-label {
    ...
    font-size: 0.7rem; font-weight: 700; color: var(--ts);
  }
  ...
  .answer-input {
    ...
    color: var(--tp); font-size: 0.95rem; font-family: var(--font-b); font-weight: 500;
  ...
  .submit-btn:not(:disabled):hover { background: #2563eb; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(59,130,246,0.3); }

  /* ── Feedback banner ── */
  .feedback-banner {
    display: flex; align-items: center; justify-content: space-between;
    padding: 14px 18px; border-radius: var(--radius-sm); margin-bottom: 16px;
    font-size: 0.85rem; font-weight: 700; font-family: var(--font-b);
  }
  ...
  .feedback-xp { font-family: var(--font-b); font-size: 0.8rem; font-weight: 800; }

  /* ── Explanation card ── */
  .ans-card {
    background: var(--bg-card); border: 1px solid var(--border2);
    border-radius: var(--radius-md); padding: 22px 24px; margin-bottom: 16px;
  }
  .ans-label { font-size: 0.65rem; color: var(--ts); letter-spacing: 1.5px; text-transform: uppercase; font-family: var(--font-b); font-weight: 700; margin-bottom: 8px; }
  .ans-value { font-size: 1.15rem; font-weight: 800; font-family: var(--font-d); color: var(--tp) !important; margin-bottom: 18px; }
  .ans-divider { height: 1px; background: var(--border); margin-bottom: 18px; }
  .ans-exp-label { font-size: 0.65rem; color: var(--ts); letter-spacing: 1.5px; text-transform: uppercase; font-family: var(--font-b); font-weight: 700; margin-bottom: 12px; }
  .step-row { display: flex; gap: 12px; align-items: flex-start; margin-bottom: 10px; }
  .step-num {
    flex-shrink: 0; width: 22px; height: 22px; border-radius: 50%;
    background: var(--bg-input); border: 1px solid var(--border2);
    display: flex; align-items: center; justify-content: center;
    font-size: 0.6rem; font-weight: 700; font-family: var(--font-b); color: var(--ts);
  }
  .step-text { font-size: 0.85rem; color: var(--tp); line-height: 1.6; font-family: var(--font-b); font-weight: 500; }

  .btn-next {
    width: 100%; padding: 16px; border-radius: var(--radius-sm);
    font-size: 0.95rem; font-weight: 700; font-family: var(--font-b);
    cursor: pointer; border: none; background: var(--bg-input); color: var(--tp);
    border: 1px solid var(--border2); transition: all var(--transition); margin-bottom: 12px;
  }
  .btn-next:hover { background: var(--bg-hover); border-color: var(--accent); color: var(--accent2); }

  .hint-row { margin-bottom: 12px; }
  .hint-btn { font-size: 0.75rem; color: var(--ts); background: var(--bg-input); border: 1px solid var(--border2); border-radius: var(--r-f); padding: 6px 14px; cursor: pointer; font-family: var(--font-b); font-weight: 600; transition: all var(--transition); }
  .hint-btn:hover { color: var(--tp); background: var(--bg-hover); border-color: var(--ts); }
  .hint-box { margin-top: 10px; padding: 12px 16px; background: rgba(59,130,246,0.08); border: 1px solid rgba(59,130,246,0.25); border-radius: var(--radius-sm); font-size: 0.85rem; color: var(--accent2); line-height: 1.5; font-family: var(--font-b); font-weight: 500; }
  ...
  /* ── STAGGERED MODE CARDS on menu enter ── */
  @keyframes cardDrop {
  ...
  /* ── Enter hint ── */
  .enter-hint {
    font-size: 0.68rem; color: var(--tm); font-family: var(--font-b);
  ...
  .combo-display, .timer-wrap, .rankup-overlay,
...
export default function MathBot() {
  ...
  const recordOutcome = useCallback((topic, isCorrect, currentStreak, currentScore) => {
    ...
    if ([5, 10, 25, 50, 100, 250, 500].includes(newSolved)) {
      addToast(`🎯 ${newSolved} questions completed! Great progress!`, 'milestone');
    }
    // Daily goal toasts
          ...
          <div className={`game-wrap fade-wrap ${
            ...
            {total > 0 && (
              <div className="accuracy-line">
                {total} completed · {Math.round((score.correct / total) * 100)}% accuracy
