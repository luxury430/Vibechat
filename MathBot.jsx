...
const STYLE = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ctrl-btn {
    ...
    color: var(--text-2); padding: 7px 12px; border-radius: 24px;
  ...
  .ctrl-btn.sound-off { color: var(--text-3); opacity: 0.6; }
  ...
  .streak-info {}
  .streak-title { font-size: 11px; color: #fb923c; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; font-family: 'JetBrains Mono', monospace; }
  .streak-days { font-size: 18px; font-weight: 800; color: var(--text); letter-spacing: -0.5px; font-family: 'JetBrains Mono', monospace; }
  .streak-right { font-size: 10px; color: var(--text-3); font-family: 'JetBrains Mono', monospace; text-align: right; line-height: 1.5; }

  /* ── DAILY CHALLENGE CARD ── */
  .daily-card {
    width: 100%; margin-bottom: 12px;
    background: linear-gradient(135deg, rgba(168,85,247,0.07) 0%, rgba(59,130,246,0.05) 100%);
  ...
  .daily-card:hover { transform: translateY(-2px); border-color: rgba(168,85,247,0.5); }
  .daily-card.completed { opacity: 0.55; cursor: default; }
  .daily-card:hover.completed { transform: none; }
  .daily-left { display: flex; align-items: center; gap: 10px; }
  .daily-icon { font-size: 20px; }
  .daily-title { font-size: 12px; font-weight: 700; color: #a855f7; letter-spacing: 1px; text-transform: uppercase; font-family: 'JetBrains Mono', monospace; }
  .daily-desc { font-size: 11px; color: var(--text-2); margin-top: 1px; }
  .daily-badge {
  ...
  .daily-badge.done { background: rgba(16,185,129,0.1); border-color: rgba(16,185,129,0.3); color: #10b981; }

  .logo { text-align: center; margin-bottom: 28px; padding-top: 12px; }
  .logo-icon {
  ...
  .solved-meter-bar { flex: 1; height: 3px; background: var(--bg-sunken); border-radius: 2px; overflow: hidden; }
  .solved-meter-fill { height: 100%; background: linear-gradient(90deg, #60a5fa, #a78bfa); border-radius: 2px; transition: width 0.6s cubic-bezier(.4,0,.2,1); }
  ...
  .top-bar {
    display: flex; align-items: center; gap: 10px;
    margin-bottom: 16px; padding-top: 2px; flex-wrap: wrap;
  }
  .back-btn {
  ...
  /* inline streak in game top bar */
  .game-streak-pill {
    margin-left: auto; display: flex; align-items: center; gap: 4px;
    font-family: 'JetBrains Mono', monospace; font-size: 10px; font-weight: 700;
    padding: 5px 10px; border-radius: 20px;
    background: rgba(251,146,60,0.08); border: 1px solid rgba(251,146,60,0.22); color: #fb923c;
  }
  ...
  .xp-section::after {
    ...
    background: radial-gradient(ellipse at 50% 100%, rgba(96,165,250,0.04) 0%, transparent 70%);
  }
  ...
  .xp-bar-bg {
    height: 5px; background: var(--bg-sunken); border-radius: 3px;
    overflow: hidden; border: 1px solid var(--border-soft); margin-bottom: 5px;
  }
  .xp-bar-fill {
  ...
  /* next-rank pill under xp bar */
  .next-rank-pill {
    font-size: 10px; font-family: 'JetBrains Mono', monospace;
    color: var(--text-3); letter-spacing: 0.3px; margin-top: 2px;
    display: flex; justify-content: space-between; align-items: center;
  }
  .next-rank-pill strong { color: var(--text-2); }
  ...
  .stat-box.pulse { animation: statPulse 0.4s ease; }
  @keyframes statPulse {
    0% { transform: scale(1); }
  ...
  .q-card-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; margin-bottom: 4px; }
  .q-topic {
  ...
  .q-text {
    ...
    letter-spacing: -0.1px; white-space: pre-wrap; color: var(--text);
  }
  /* daily challenge glow on q-card */
  .q-card.daily-glow {
    border-color: rgba(168,85,247,0.4);
    box-shadow: 0 0 0 1px rgba(168,85,247,0.08), 0 0 24px rgba(168,85,247,0.06), var(--shadow-card);
  }
  ...
  .timer-ring-fill { fill: none; stroke-width: 2.5; stroke-linecap: round; transition: stroke-dashoffset 1s linear, stroke 0.5s ease; }
  .timer-num {
  ...
  .timer-label { font-family: 'JetBrains Mono', monospace; font-size: 8px; color: var(--text-3); letter-spacing: 1px; text-transform: uppercase; }
  /* speed bonus badge */
  .speed-badge {
  ...
  .today-bar-label { color: var(--text-3); white-space: nowrap; }
  .today-bar-track { flex: 1; height: 3px; background: var(--bg-sunken); border-radius: 2px; overflow: hidden; }
  .today-bar-fill { height: 100%; background: linear-gradient(90deg, #3b82f6, #a855f7); border-radius: 2px; transition: width 0.5s ease; }
  .today-bar-count { color: var(--text-2); font-weight: 700; white-space: nowrap; }

  /* ── Options toggle ── */
  .options-toggle-row {
    display: flex; align-items: center; justify-content: space-between;
    padding: 12px 16px; background: var(--bg-raised); border: 1px solid var(--border);
    border-radius: var(--radius-md); margin-bottom: 12px; box-shadow: var(--shadow-sm);
  }
  .options-toggle-label { font-size: 13px; font-weight: 600; }
  .options-toggle-desc { font-size: 11px; color: var(--text-3); margin-top: 2px; font-family: 'JetBrains Mono', monospace; }
  .toggle-switch { position: relative; display: inline-block; width: 40px; height: 22px; flex-shrink: 0; }
  .toggle-switch input { opacity: 0; width: 0; height: 0; }
  .toggle-track {
    position: absolute; inset: 0; background: var(--bg-sunken); border: 1px solid var(--border);
    border-radius: 11px; cursor: pointer; transition: background 0.2s ease;
  }
  .toggle-track::before {
  ...
  .toggle-switch input:checked + .toggle-track { background: rgba(59,130,246,0.35); border-color: rgba(59,130,246,0.5); }
  .toggle-switch input:checked + .toggle-track::before { transform: translateX(18px); background: #3b82f6; }

  /* ── Hint ── */
  .hint-row { margin-bottom: 12px; }
  .hint-btn {
    font-size: 11px; color: var(--text-3); background: transparent; border: 1px solid var(--border);
    border-radius: 20px; padding: 5px 12px; cursor: pointer; font-family: 'JetBrains Mono', monospace;
    font-weight: 600; transition: all var(--transition); letter-spacing: 0.5px;
  }
  .hint-btn:hover { color: var(--text-2); border-color: var(--text-3); }
  .hint-box {
  ...
  .option-btn:not(:disabled):hover { border-color: var(--text-3); transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,0.3); }
  .option-btn:not(:disabled):active { transform: scale(0.98); }
  .option-btn.opt-correct { border-color: rgba(16,185,129,0.6); background: rgba(16,185,129,0.08); color: #10b981; }
  .option-btn.opt-wrong   { border-color: rgba(239,68,68,0.5);  background: rgba(239,68,68,0.06);  color: #f87171; }
  .option-label {
  ...
  .answer-input:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,0.12); }
  .answer-input.ev-correct { border-color: rgba(16,185,129,0.6); box-shadow: 0 0 0 3px rgba(16,185,129,0.1); }
  .answer-input.ev-wrong   { border-color: rgba(239,68,68,0.5);  box-shadow: 0 0 0 3px rgba(239,68,68,0.08); }
  .answer-input.ev-partial { border-color: rgba(251,191,36,0.5); box-shadow: 0 0 0 3px rgba(251,191,36,0.08); }
  .submit-btn {
    padding: 12px 20px; border-radius: var(--radius-sm); border: 1px solid rgba(59,130,246,0.4);
    background: rgba(59,130,246,0.12); color: #60a5fa; font-size: 13px; font-weight: 700;
    font-family: 'JetBrains Mono', monospace; cursor: pointer; letter-spacing: 0.3px;
    transition: all var(--transition); box-shadow: var(--shadow-sm); white-space: nowrap;
  }
  .submit-btn:not(:disabled):hover { background: rgba(59,130,246,0.22); border-color: rgba(59,130,246,0.6); transform: translateY(-1px); }
  .submit-btn:disabled { opacity: 0.4; cursor: not-allowed; }
  .enter-hint { font-size: 10px; color: var(--text-3); font-family: 'JetBrains Mono', monospace; margin-top: 4px; margin-bottom: 10px; letter-spacing: 0.3px; }
  ...
  .feedback-banner {
    ...
    animation: bannerIn 0.25s cubic-bezier(.34,1.56,.64,1);
  }
  @keyframes bannerIn {
    0% { opacity: 0; transform: translateY(-6px) scale(0.97); }
    100% { opacity: 1; transform: translateY(0) scale(1); }
  }
  .ev-correct { background: rgba(16,185,129,0.08); border: 1px solid rgba(16,185,129,0.3); color: #10b981; }
  .ev-wrong   { background: rgba(239,68,68,0.07);  border: 1px solid rgba(239,68,68,0.25);  color: #f87171; }
  .ev-partial { background: rgba(251,191,36,0.07); border: 1px solid rgba(251,191,36,0.25); color: #fbbf24; }
  .feedback-xp { font-family: 'JetBrains Mono', monospace; font-size: 12px; font-weight: 800; letter-spacing: 0.5px; }
  .feedback-right { display: flex; flex-direction: column; align-items: flex-end; gap: 3px; }
  ...
  .ans-label { font-size: 9px; color: var(--text-3); letter-spacing: 2.5px; text-transform: uppercase; font-family: 'JetBrains Mono', monospace; font-weight: 700; margin-bottom: 6px; }
  .ans-value { font-size: 18px; font-weight: 700; font-family: 'JetBrains Mono', monospace; letter-spacing: -0.5px; margin-bottom: 14px; }
  .ans-divider { height: 1px; background: var(--border-soft); margin-bottom: 14px; }
  .ans-exp-label { font-size: 9px; color: var(--text-3); letter-spacing: 2.5px; text-transform: uppercase; font-family: 'JetBrains Mono', monospace; font-weight: 700; margin-bottom: 10px; }
  ...
  .step-num {
    ...
    background: var(--bg-sunken); border: 1px solid var(--border-soft);
    display: flex; align-items: center; justify-content: center;
    font-size: 9px; font-weight: 700; font-family: 'JetBrains Mono', monospace; color: var(--text-3);
  }
  .step-text { font-size: 13px; color: var(--text-2); line-height: 1.6; font-family: 'JetBrains Mono', monospace; }

  /* ── Next button ── */
  .btn-next {
    width: 100%; padding: 14px; border-radius: var(--radius-md);
    font-size: 14px; font-weight: 700; font-family: 'Inter', sans-serif;
    cursor: pointer; letter-spacing: 0.3px; border: 1px solid;
    transition: all var(--transition); margin-bottom: 8px;
    display: flex; align-items: center; justify-content: center; gap: 8px;
  }
  .btn-next:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.3); }
  ...
  /* ═══════════════════════════════════════════════════
     🎮 ADDICTIVE MECHANICS STYLES
  ═══════════════════════════════════════════════════ */

  /* ── XP BURST (floating +XP animation) ── */
  @keyframes xpFloat {
  ...
  .xp-burst {
    ...
    animation: xpFloat 1.1s cubic-bezier(.2,.8,.3,1) forwards;
    white-space: nowrap; text-shadow: 0 2px 12px currentColor;
    font-size: 22px;
  }
  ...
  .combo-display {
    ...
    animation: comboPop 0.35s cubic-bezier(.34,1.56,.64,1);
    border: 1px solid;
  }
  .combo-display.combo-3  { background: rgba(251,191,36,0.1); border-color: rgba(251,191,36,0.35); color: #fbbf24; }
  .combo-display.combo-5  { background: rgba(239,68,68,0.1);  border-color: rgba(239,68,68,0.35);  color: #f87171; animation: comboPop 0.35s cubic-bezier(.34,1.56,.64,1), comboShake 0.4s ease 0.3s; }
  .combo-display.combo-7  { background: rgba(168,85,247,0.12); border-color: rgba(168,85,247,0.4); color: #c084fc; animation: comboPop 0.35s cubic-bezier(.34,1.56,.64,1), comboShake 0.4s ease 0.3s; }

  /* ── RANK-UP MODAL ── */
  @keyframes rankUpOverlayIn {
    0%   { opacity: 0; }
    100% { opacity: 1; }
  }
  @keyframes rankUpCardIn {
  ...
  .rankup-overlay {
    ...
    display: flex; align-items: center; justify-content: center;
    animation: rankUpOverlayIn 0.3s ease;
  ...
  .rankup-card {
    ...
    animation: rankUpCardIn 0.55s cubic-bezier(.34,1.56,.64,1) forwards;
    box-shadow: 0 32px 80px rgba(0,0,0,0.8), 0 0 60px rgba(168,85,247,0.1);
    max-width: 340px; width: 100%; position: relative; overflow: hidden;
  }
  .rankup-card::before {
    content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px;
    background: linear-gradient(90deg, #3b82f6, #a855f7, #ec4899, #3b82f6);
    background-size: 200% auto;
    animation: rankUpShine 2s linear infinite;
  }
  .rankup-emoji { font-size: 52px; margin-bottom: 14px; display: block; }
  .rankup-label { font-size: 10px; color: var(--text-3); letter-spacing: 3px; text-transform: uppercase; font-family: 'JetBrains Mono', monospace; font-weight: 700; margin-bottom: 10px; }
  .rankup-title { font-size: 26px; font-weight: 800; letter-spacing: -0.5px; margin-bottom: 8px; }
  .rankup-sub { font-size: 13px; color: var(--text-2); line-height: 1.5; margin-bottom: 24px; }
  .rankup-close {
    padding: 12px 28px; border-radius: 12px; border: 1px solid; font-size: 13px; font-weight: 700;
    font-family: 'Inter', sans-serif; cursor: pointer; transition: all var(--transition);
    letter-spacing: 0.3px;
  }
  .rankup-close:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.3); }
  /* particles inside rank-up card */
  .ru-particles { position: absolute; inset: 0; pointer-events: none; overflow: hidden; }
  .ru-particle {
    position: absolute; width: 5px; height: 5px; border-radius: 50%;
    animation: ruParticle 1.5s ease-out forwards;
  }
  @keyframes ruParticle {
  ...
  .toast-container {
    ...
    z-index: 9998; display: flex; flex-direction: column-reverse; gap: 8px;
    pointer-events: none; align-items: center; width: 90%; max-width: 380px;
  }
  @keyframes toastIn {
  ...
  .toast.success { background: rgba(16,185,129,0.12); border-color: rgba(16,185,129,0.35); color: #34d399; }
  .toast.wrong   { background: rgba(239,68,68,0.1);   border-color: rgba(239,68,68,0.3);   color: #f87171; }
  .toast.info    { background: rgba(59,130,246,0.1);   border-color: rgba(59,130,246,0.3);  color: #60a5fa; }
  .toast.fire    { background: rgba(251,146,60,0.1);   border-color: rgba(251,146,60,0.3);  color: #fb923c; }
  .toast.milestone { background: rgba(168,85,247,0.1); border-color: rgba(168,85,247,0.3); color: #c084fc; }
  .toast.daily   { background: rgba(168,85,247,0.12); border-color: rgba(168,85,247,0.4); color: #a855f7; }

  /* ── DAILY CHALLENGE ACTIVE banner (in game) ── */
  .dc-active-banner {
  ...
  .rank-pass-banner {
    ...
    animation: toastIn 0.35s cubic-bezier(.34,1.56,.64,1), toastOut 0.3s ease 2.5s forwards;
    white-space: nowrap;
  }
`;
...
export default function MathBot() {
  ...
  const recordOutcome = useCallback((topic, isCorrect, currentStreak, currentScore) => {
    ...
    if ([5, 10, 25, 50, 100, 250, 500].includes(newSolved)) {
      addToast(`🎯 ${newSolved} questions solved! Keep grinding!`, 'milestone');
    }
    // Daily goal toasts
    ...
    if (isCorrect) {
      ...
      // Speed bonus: if answered with > 50% time remaining, +15% bonus XP
      let bonus = 0;
          ...
          <div className="game-wrap fade-wrap" style={{ opacity: anim ? 0.25 : 1 }}>
            ...
            {isDailyQ && !dcDone && (
              <div className="dc-active-banner">
                ⭐ Daily Challenge · Correct answer earns +50 XP bonus!
              </div>
            )}
                ...
                {evaluation && (
                  <div className={`feedback-banner ${evClass}`}>
                    <span>
                      ...
                      {evaluation === "wrong"   && "✗ Wrong — study the steps below"}
