/* globals: React is set by the module script in chat.html */

// ── Safely get React hooks at runtime. Babel can race with the module
//    script that sets window.React; using a getter ensures we never
//    destructure from undefined and crash the whole component tree. ──
const _getReact = () => window.React || {};
const { useState, useCallback, useRef, useEffect, useMemo } = _getReact();
...
function MathBot() {
  // ── Block render until window.React is confirmed ready.
  //    Normally instant; protects against the Babel/module-script race. ──
  const [reactReady, setReactReady] = useState(
    () => !!(window.React && window.React.useState)
  );
  useEffect(() => {
    if (reactReady) return;
    const iv = setInterval(() => {
      if (window.React && window.React.useState) { clearInterval(iv); setReactReady(true); }
    }, 30);
    const to = setTimeout(() => { clearInterval(iv); setReactReady(true); }, 5000); // failsafe
  ...
  if (!reactReady) {
    return (
      <div style={{ display:"flex", justifyContent:"center", alignItems:"center",
                    ...
                    background:"var(--bg-chat,#0d1117)", color:"#60a5fa",
                    fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor"
             strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
             style={{ animation:"mb-spin .8s linear infinite" }}>
          <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
        </svg>
        <style>{`@keyframes mb-spin{to{transform:rotate(360deg)}}`}</style>
        <span style={{ fontSize:13 }}>Initializing MathBot…</span>
      </div>
    );
  }
...
// Register for use by openMath()
window._MathBotComponent = MathBot;
