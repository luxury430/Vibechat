...
`;
  ...
  function _buildCard(card, isSender, cardUid) {
    ...
    /* reply button — only for the receiver */
    const replyBtn = !isSender
...
LaTeX is supported: $x^2$ for inline, $$E=mc^2$$ for display."></textarea>
  ...
  async function _sendReply() {
    ...
    /* Build the reply text with a clean academic header */
    const card   = _replyCard;
    const type   = card ? (card.type||'question') : 'question';
    const meta   = TYPE_META[type] || TYPE_META.question;
    const topic  = card ? (card.topic||card.chapter||'General') : 'General';
    const replyLabel = card
      ? (card.equation ? card.equation.slice(0,50) : (card.title || (card.body||'').slice(0,60)))
      : 'Study card';

    /* Set reply context (so it shows as a quoted reply in the chat) */
    if (typeof window.doReply === 'function') {
      window.doReply(_replyCardUid.replace('dc_',''), `[${meta.label}] ${_e(replyLabel).slice(0,55)}`);
    }
    ...
    if (msgInp) {
      msgInp.value = text;
      msgInp.dispatchEvent(new Event('input', { bubbles: true }));
    }
    ...
    if (typeof window.sendMsg === 'function') {
      try {
        ...
        console.error('[dcPatch] reply send error:', e);
        if (typeof window.toast === 'function') window.toast('Failed to send reply');
        send.disabled  = false;
